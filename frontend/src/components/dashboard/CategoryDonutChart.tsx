...
export const CategoryDonutChart: React.FC<CategoryDonutChartProps> = ({ data }) => {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-slate-900 border border-slate-700 p-3 rounded-xl shadow-xl text-xs space-y-1">
          ...
          <p className="text-emerald-400 font-bold text-sm">{formatGHS(item.amount)}</p>
          <p className="text-slate-400">{item.percentage}% of spending ({item.count} transactions)</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="p-5 sm:p-6 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-sm flex flex-col">
      <div className="mb-2">
        <h3 className="text-base font-bold text-white tracking-tight">Spending by Category</h3>
        <p className="text-xs text-slate-400">Distribution of recent expense categories</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center flex-1">
        ...
        <div className="sm:col-span-6 h-56 relative flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className="text-sm font-bold text-white tracking-tight">{formatGHS(totalExpense)}</span>
          </div>
        </div>
          ...
          {data.slice(0, 7).map((item, idx) => (
            <div key={idx} className="flex items-center justify-between text-xs py-1 border-b border-slate-800/60 last:border-0">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className="font-semibold text-white">{formatGHS(item.amount)}</span>
                <span className="text-[10px] text-slate-400 font-mono w-9 text-right">{item.percentage}%</span>
              </div>
            </div>
          ))}
        </div>
