...
export const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  return (
    <div className="space-y-5 sm:space-y-6 pb-20 lg:pb-12">
      ...
      {accounts.length === 0 && (
        <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-r from-emerald-50 via-white to-amber-50 dark:from-emerald-950/60 dark:via-slate-900 dark:to-amber-950/40 border border-emerald-200 dark:border-emerald-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
          <div>
            <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white tracking-tight">Welcome to CediTrack Ghana!</h3>
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            <button
              onClick={onSeedData}
              disabled={seeding}
              className="px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold transition-all shadow"
            >
              {seeding ? 'Seeding...' : 'Seed Sample Ghana Data'}
            </button>
            <button
              onClick={() => onNavigateTab('accounts')}
              className="px-4 py-2.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-900 dark:text-white text-xs font-semibold border border-slate-200 dark:border-slate-700 transition-all shadow-sm"
            >
              Add Accounts
            </button>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard
          ...
          subtitle={`Across ${summary?.accounts_count || 0} accounts`}
          icon={Wallet}
          variant="emerald"
        />
        <StatCard
        ...
        <StatCard
          ...
          subtitle={`${summary?.savings_rate || 0}% monthly rate`}
          icon={PiggyBank}
          variant="amber"
        />
      </div>
      ...
      {accounts.length > 0 && (
        <div className="space-y-2.5">
          <div className="flex items-center justify-between">
            <h4 className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Account Balances</h4>
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => onNavigateTab('accounts')}
                className="text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 transition-colors"
              >
                Manage
              </button>
            </div>
            ...
            {accounts.map((acc) => {
              ...
              return (
                <div
                  ...
                  className="p-3 sm:p-3.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-emerald-500/40 cursor-pointer transition-all flex flex-col justify-between shadow-sm"
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-semibold text-slate-900 dark:text-white truncate">{acc.name}</span>
                  </div>
                  <div>
